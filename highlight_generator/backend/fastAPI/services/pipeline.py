...
from event_detection.event_classifier import (
    classify_scene_types,
    match_audio_and_scene,
    save_highlights,
)
...
def run_highlight_pipeline(video_path: str, output_path: str = None) -> str:
    frame_dir = os.path.join(BASE_DIR, "data", "frames")
    model_dir = os.path.join(BASE_DIR, "models")
    output_dir = os.path.join(BASE_DIR, "output")
    highlights_path = os.path.join(output_dir, "highlights.json")
    print("[🎬] 🔁 Running highlight pipeline")

    # Ensure output_path is defined
    if output_path is None:
        output_path = os.path.join(output_dir, f"highlight_{os.path.basename(video_path)}")
    ...
    # Step 2: Load or build BoVW model
    ...
    # Step 3: Load or train classifier
    ...
    # Step 4: Run scene classification FIRST (no threads!)
    frame_scenes = classify_scene_types(frame_dir, kmeans_model, clf)

    # Step 5: Run audio extraction in parallel
    ...
    # Step 5: Match events & save highlight metadata
    highlights = match_audio_and_scene(spikes, frame_scenes)
    save_highlights(highlights, highlights_path)

    # Step 6: Generate final highlight video
    render_highlight_video(video_path, highlights_path, output_path)
